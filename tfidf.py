...
reviews = session.read.csv("hdfs:///DBProject/review.csv", header=False, sep=",", schema=schema)
reviews = reviews.na.drop(subset=["reviewText"])    # drop the reviews with NA reviewText
...
# get tf
cv = CountVectorizer(inputCol="reviewWords", outputCol="rawFeatures", vocabSize=20)
...
output_df = session.createDataFrame(output, ['reviewerID', 'asin', 'createdAt', 'tfidf'])
